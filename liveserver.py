...
from locapp.shared.liveshared import pull_near_me, do_first_pull
...
@app.route('/dofirstpull/', methods=['GET'])
def dofirstpull():
    passw = request.args.get("key")

    if passw != '1234':
        status = {}
        status['status'] = 400
        status['msg'] = "failed with the key"
